...
namespace Nexcess\WooBetterReviews\Tables\AuthorMeta;
...
function insert_row( $insert_args = array() ) {
	...
	// Do the validations.
	Database\validate_insert_args( 'authormeta', $insert_args ); // @@todo better return?
	...
	$wpdb->insert( $wpdb->wc_better_rvs_authormeta, $insert_args, $table_format );
...
function update_row( $update_id = 0, $update_args = array(), $return_bool = true ) {
	...
	// Do the validations.
	Database\validate_update_args( 'authormeta', $update_args ); // @@todo better return?

	// Call the global DB.
	global $wpdb;

	// Set our table formatting.
	$table_format   = Database\set_update_format( 'authormeta', $update_args );

	// Run the update process.
	$wpdb->update( $wpdb->wc_better_rvs_authormeta, $update_args, array( 'ameta_id' => absint( $update_id ) ), $table_format, array( '%d' ) );
...
function delete_row( $delete_id = 0 ) {
	...
	$wpdb->delete( $wpdb->wc_better_rvs_authormeta, array( 'ameta_id' => absint( $delete_id ) ) );
